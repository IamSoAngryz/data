"""
Lambda functions (lamda expressions) = functii anonime
...
care se comporta ca o funtie definita cu cuvantul cheie def
Avantaje:
    - readability
    - codul este concis
    - economie de memorie
...
print(f'Valaorea 4 ridicata la puterea a doua: {square(4)}')
print(f'Valaorea 4 ridicata la puterea a doua: {(lambda x: x**2)(4) }')
...
print(type(patrat))
m2 = map(patrat, nums)
print(f'Valoarea listei {nums} ridicate la puterea a 2-a: {list(m2)}')
print(
    f'Valoarea listei {nums} ridicate la puterea a 2-a: {list(map(lambda x: x**2, nums))}')
