...
(Utilizare numpy in calcule matematice)
- realizeaza operatile element cu element
    - numpy opereaza elementwise (element cu element)
    - aplica functii matematice ficarui element din array (tablou)
"""
...
print('Functia logaritm aplicata unui array:')
print(np.log(new_ar))
print(np.cos(new_ar))
...
print(b)
c = np.arange(2, 11)
b2 = np.linspace(2, 10, 20)     # dif_2_valori_consecutive = (stop - start)/(nr_elem-1)
print(b2)
...
print(f'Suma fiecarei linii din new_ar: {new_ar.sum(axis=1)}') # 0 coloane